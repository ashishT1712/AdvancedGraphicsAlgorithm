/*
* Vulkan Example - Implements a separable two-pass fullscreen blur (also known as Pyramid)
*
...
public:
	...
	struct {
		vks::TexturepyrmaidMap pyrmaidmap;
	} textures;
	...
	{
		title = "Pyramid (offscreen rendering)";
		timerSpeed *= 0.5f;
	...
	void loadAssets()
	{
		...
		models.skyBox.loadFromFile(getAssetPath() + "models/pyramid.obj", vertexLayout, 1.0f, vulkanDevice, queue);
		textures.pyrmaidmap.loadFromFile(getAssetPath() + "textures/pyrmaidmap_space.ktx", VK_FORMAT_R8G8B8A8_UNORM, vulkanDevice, queue);
	}
	...
	void setupDescriptorSet()
	{
		...
		writeDescriptorSets = {			
			...
			vks::initializers::writeDescriptorSet(descriptorSets.skyBox, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,	1, &textures.pyrmaidmap.descriptor),					// Binding 1: Fragment shader texture sampler
		};
		vkUpdateDescriptorSets(device, writeDescriptorSets.size(), writeDescriptorSets.data(), 0, NULL);
	}
	...
	void preparePipelines()
	{
		...
		// Blur pipelines
		shaderStages[0] = loadShader(getAssetPath() + "shaders/Pyramid/Tiwari_4.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
		shaderStages[1] = loadShader(getAssetPath() + "shaders/Pyramid/Tiwari_4.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
		...
		shaderStages[0] = loadShader(getAssetPath() + "shaders/Pyramid/Tiwari_1.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
		shaderStages[1] = loadShader(getAssetPath() + "shaders/Pyramid/Tiwari_1.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
		...
		shaderStages[0] = loadShader(getAssetPath() + "shaders/Pyramid/Tiwari_2.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
		shaderStages[1] = loadShader(getAssetPath() + "shaders/Pyramid/Tiwari_2.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
		pipelineCreateInfo.renderPass = offscreenPass.renderPass;
		VK_CHECK_RESULT(vkCreateGraphicsPipelines(device, pipelineCache, 1, &pipelineCreateInfo, nullptr, &pipelines.glowPass));

		// Skybox (pyrmaidmap)
		shaderStages[0] = loadShader(getAssetPath() + "shaders/Pyramid/Tiwari_3.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
		shaderStages[1] = loadShader(getAssetPath() + "shaders/Pyramid/Tiwari_3.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
	...
	virtual void OnUpdateUIOverlay(vks::UIOverlay *overlay)
	{
		if (overlay->header("Settings")) {
			if (overlay->checkBox("Pyramid", &Pyramid)) {
				buildCommandBuffers();
			}
